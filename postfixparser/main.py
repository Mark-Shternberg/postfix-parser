...
async def import_log(logfile: str) -> Dict[str, PostfixMessage]:
    ...
    with open(logfile, 'r') as f:
        while True:
            ...
            date = date_converter(dtime, "date")
            date_full = date_converter(dtime, "date_full")
            if qid not in messages:
                messages[qid] = PostfixMessage(timestamp=dtime, queue_id=qid, date=date, date_full=date_full)
    ...
    with open(logfile, 'r') as g:
        while True:
            ...
            date = date_converter(dtime2, "date")
            date_full = date_converter(dtime2, "date_full")
            today = datetime.date.today()
            dtime = dtime2 + str(today.year)
            noqid = date_converter(dtime, "id")
            if noqid not in messages:
                messages[noqid] = PostfixMessage(timestamp=dtime2, queue_id=noqid, date=date, date_full=date_full)
...
def date_converter(ldate: str,i: str):
    if 'Jan' in ldate: 
    ...
    if i == 'date':
        ...
        date = ldate + " " + str(today.year)
        date = date.replace("  "," ")
        date = date.split(' ')[1]+"-"+date.split(' ')[0]+"-"+date.split(' ')[3]+" "+date.split(' ')[2]
        return date
    else: return ldate